...
class MainPageForm:
    ...
    def is_fields_green(self) -> bool:
        '''
        Check is field green
        '''
        is_true = False
        fields = self.browser.\
            find_elements(By.XPATH, '//*[@id and not (@id="zip-code")]')
        for field in fields:
            if field.value_of_css_property('background-color') \
                    == 'rgba(209, 231, 221, 1)':
                is_true = True
            else:
                is_true = False
    ...
    def is_zip_code_red(self) -> bool:
        '''
        Check is zip code field red
        '''
        zip_field = self.browser.find_element(By.ID, 'zip-code')
        if zip_field.value_of_css_property('background-color') \
                == 'rgba(248, 215, 218, 1)':
            return True
        else:
            return False
